...
function eagerFetch(builder, $models) {
  if ($models instanceof builder._modelClass || _lodash2.default.isArray($models) && $models[0] instanceof builder._modelClass) {
    var _ret = function () {
      ...
      var filters = _lodash2.default.clone(builder._eagerFilters || {});

      _lodash2.default.each(builder._eagerFilterExpressions, function (filter, idx) {
        var filterNodes = expression.expressionsAtPath(filter.path);

        if (!_lodash2.default.isEmpty(filterNodes)) {
          (function () {
            var filterName = '_efe' + idx + '_';
...
function callOnBuildHooks(builder, func) {
  if (_lodash2.default.isFunction(func)) {
    ...
    _lodash2.default.each(func, function (func) {
      func.call(builder, builder);
    });
  }
}
